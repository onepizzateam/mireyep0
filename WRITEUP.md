...
60 fields across 6 layers combine into a single logical request — two parallel `/v1/fetch` batch calls (Mireye enforces a 50-field cap per call, so the 60 fields split into two 30-field batches fired simultaneously; wall time is the slower batch, not the sum). No stitching, no geocoding pipeline, no manual data acquisition.

---

## 4. The 60 Fields and Why They Matter
...
SignalRent is a web application. A landlord enters a US address, the carrier or tower company name, and optionally their current or offered lease rate. SignalRent calls Mireye `/v1/fetch` with the 60 relevant fields (split into two parallel 30-field batch requests to stay under Mireye's 50-field-per-call limit), runs the scoring model, and returns:
...
### Location input: address search + draggable pin

Address-string geocoding is not precise enough for this use case. A geocoder returns a point interpolated from road centerline data — it knows the street and roughly which block, but it doesn't know the parcel boundary or where on that parcel a tower actually sits. For a large rural parcel, that can put the fetched coordinate hundreds of meters from the actual site, which materially changes fields like `nearest_antenna_structure_distance_m`, `slope_degrees`, or `within_floodplain_polygon`.

SignalRent's input flow reflects this: the user types an address, which flies the map to that location and drops a pin at the geocoded point via Mapbox's geocoder. The user then drags the pin to the exact spot — the parcel corner, the road edge, wherever the tower actually is or would be. It is the confirmed pin coordinate, not the raw geocoded coordinate, that is sent to Mireye's `/v1/fetch` call. This matters because the actual users of this product (landlords who've received a site acquisition letter) typically already know exactly which corner of their property is in question — the product should let them specify that precisely rather than trusting an address-matching guess.

### User scenarios

**User A — new lease offer**
Margaret, a retired farmer outside Flagstaff, Arizona, receives a letter from a Crown Castle site acquisition agent offering $750/month for a 30-year ground lease on the corner of her property. She goes to SignalRent.com, enters her parcel address, drags the pin to the exact corner named in the letter, types "Crown Castle," and enters $750. The tool fetches her Mireye data: `antenna_structures_within_500m_count = 0`, `antenna_structures_within_2km_count = 1` (that one is a guyed tower already at capacity per structure type), `mobile_5g_coverage_class = "No coverage"`, `within_floodplain_polygon = false`, `intersects_critical_habitat = false`, `parcel_zoning = "Agricultural"`, `elevation = 2,134m`. Her site scores 81/100 with a leverage multiplier of 1.4×. Benchmark range: $1,650–$2,400/month. Leverage summary: "Crown Castle has no viable alternative within the standard search ring. The nearest structure is a guyed tower — structure type suggests additional co-location may be possible, but you should verify current tenancy before conceding on that point. Your elevation advantage at 2,134m expands coverage radius significantly. Leverage is high." Margaret counters at $1,800.
...
**Free tier**: Address (pin-confirmed location) → site score + dimension breakdown + benchmark range

**Paid tier ($49 one-time per report)**: A generated PDF report — the full negotiation-ready deliverable — containing:
- **Cover section**: property address, confirmed coordinates, carrier/tower company name, date generated
- **Site score summary**: 0–100 score, full dimension breakdown, and the leverage multiplier with a plain-English explanation of what it means for the negotiation
- **Benchmark range**: monthly rent range, and — if a current or offered rate was entered — exactly how far above or below benchmark it falls
- **Field-level breakdown table**: every Mireye field used, its fetched value, which dimension it feeds, and why it matters (drawn directly from the field tables in Section 4, so the report and the underlying methodology never diverge)
- **Leverage summary**: the negotiating-position paragraph, expanded with specific field citations rather than the shorter free-tier version
- **Data provenance & limitations**: which fields came back null or unavailable for this specific site, plus the standing caveats from Section 8 (the FCC tenancy gap chief among them) — scoped to what actually applies to that address, not generic boilerplate
...
The paid report is the primary revenue driver. The free tier exists to demonstrate the product's value and create the moment where a landlord realizes how undervalued their lease is — that is the conversion event. Making the report a literal generated PDF (not just a "coming soon" tier on a pricing page) matters for this submission specifically: it closes the loop from address input to a real, deliverable artifact a landlord could hand to their attorney, rather than stopping at a scoring demo.
...
The competitive moat is not the scoring model — that is replicable. The moat is: (1) the Mireye integration that collapses a 12-source data pull into two parallel API calls, and (2) outcome data. Every time a SignalRent user reports back what they actually negotiated, that data calibrates the model. Over time, SignalRent's benchmark ranges become empirically grounded in real negotiated rates rather than industry-published approximations. That feedback loop is not replicable by a consultant with a spreadsheet.
...
Until that field exists, SignalRent handles this gap two ways in the product itself. First, the fetched-field disclosure (surfaced in the app's side panel and in the paid PDF report) flags structure type with a note: "structure type suggests co-location capacity; verify current tenancy independently," and links the FCC ASR registration number directly to the FCC ULS search page so a landlord can check tenancy themselves in about a minute — turning a data gap into a usable feature rather than a silent blind spot. Second, the scoring model downweights the competitive density signal for guyed tower alternatives vs. monopoles, since guyed towers have higher theoretical capacity even if actual tenancy is unknown. Both are stopgaps, not fixes — the report is explicit with the user that this is the single largest source of uncertainty in the score.
...
### Fiber backhaul is a proxy, not a direct measurement

`fiber_broadband_available` and `fiber_provider_count` are used in the Construction Cost dimension as a signal for backhaul cost — no fiber nearby means the carrier likely needs microwave backhaul equipment, which is a real cost premium. But these fields, sourced for residential/commercial broadband availability, don't directly measure whether carrier-grade dark fiber or backhaul-capable fiber runs along the access road to the site. A landlord on a road with only consumer broadband but no carrier backhaul route may see an overstated leverage signal in the Construction Cost dimension, since the model currently can't distinguish "fiber exists nearby" from "fiber exists and is backhaul-capable." This is disclosed in the paid report's limitations section for any site where these fields are the primary driver of the construction-cost score.
...
### Out of scope for any automated tool

Two further gaps surfaced in review are real, but worth naming as inherent limits of this category of product rather than something Mireye specifically is missing: spectrum holdings by market at the tower-site level (no public data source maps this at parcel resolution), and HOA or local political litigation history around tower siting (no data source publishes this in structured form). Both would require carrier-internal or legal-research-grade data no infrastructure API exposes. They're noted here for completeness, not as a Mireye roadmap item.

---

*SignalRent — built on Mireye. 60 fields, one request, one negotiation you don't lose.*