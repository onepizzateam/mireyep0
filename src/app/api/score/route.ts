...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const state = await graph.invoke({ address: obj.address.trim(), lat: typeof obj.lat === "number" ? obj.lat : undefined, lng: typeof obj.lng === "number" ? obj.lng : undefined, carrier: typeof obj.carrier === "string" ? obj.carrier : undefined, offeredRate: typeof obj.offeredRate === "number" ? obj.offeredRate : undefined, buyoutAmount: typeof obj.buyoutAmount === "number" ? obj.buyoutAmount : undefined, resolvedLat: 0, resolvedLng: 0, displayAddress: "", evidence: [], capabilities: [], plannerOutput: [], executorOutput: [], result: null, error: null });
    console.info("[reasoning] graph completed", { resultPresent: Boolean(state.result), error: state.error ?? null, evidenceCount: state.evidence?.length ?? 0 });
