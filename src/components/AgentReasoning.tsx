export function AgentReasoning({ reasoning, evidence }: { reasoning?: string; evidence?: { fieldsFetched: number; fieldsNull: number } }) {
  ...
  return <div className="w-full max-w-2xl mx-auto bg-white border border-gray-200 rounded-lg shadow-sm"><div className="px-6 py-4 border-b"><h3 className="text-sm font-semibold text-gray-900">Agent Reasoning</h3><p className="text-xs text-gray-900 font-mono mt-1">How the valuation engine interpreted this site&apos;s data</p></div><div className="px-6 py-4"><p className="text-xs text-gray-900 leading-relaxed whitespace-pre-wrap font-mono">{cleanReasoning}</p>{evidence && <p className="text-xs text-gray-900 mt-3 font-mono">{evidence.fieldsFetched} fields fetched · {evidence.fieldsNull} returned null</p>}</div></div>;
}
